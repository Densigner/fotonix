...
function mergeSmallRegions(regionMap, regions, colorAssign, palette, width, height, minArea, edgeMap = null) {
  ...
  // ── Dark-feature protection ───────────────────────────────────────────
  //    Small very-dark regions (pupils, nostrils) surrounded by much lighter
  //    neighbours must be preserved — skip them entirely.
  function isDarkFeature(region) {
    if (region.area >= 200) return false;
    const col = palette[region.colorIndex];
    const lum = 0.299 * col.r + 0.587 * col.g + 0.114 * col.b;
    return lum < 60;
  }

  function allNeighboursLighter(region) {
    const col = palette[region.colorIndex];
    const rLum = 0.299 * col.r + 0.587 * col.g + 0.114 * col.b;
    for (const px of region.pixels) {
      ...
      for (const n of adj) {
        ...
        if (nId === region.id || nId === -1) continue;
        const nRegion = regionById.get(nId);
        if (!nRegion || nRegion.area === 0) continue;
        const nc = palette[nRegion.colorIndex];
        const nLum = 0.299 * nc.r + 0.587 * nc.g + 0.114 * nc.b;
        if (nLum - rLum <= 30) return false;
      }
    }
    return true;
  }
  ...
  while (changed) {
    ...
    for (const small of smallRegions) {
      ...
      // Protect small dark features (pupils, nostrils) from being merged
      if (isDarkFeature(small) && allNeighboursLighter(small)) continue;
...
function mergeSimilarNeighbours(regionMap, regions, palette, width, height, deltaEThreshold = 6, smallArea = 400, eyeMask = null, edgeMap = null, zoneMap = null) {
  ...
  while (changed) {
    ...
    for (const small of sorted) {
      ...
      // Protect small dark features (pupils, nostrils) from being merged
      if (small.area < 400) {
        const sCol = palette[small.colorIndex];
        const sLum = 0.299 * sCol.r + 0.587 * sCol.g + 0.114 * sCol.b;
        if (sLum < 80) {
          const neighbours = adjacency.get(small.id);
          let allLighter = neighbours && neighbours.size > 0;
          if (allLighter) {
            for (const nId of neighbours) {
              const nr = regionById.get(nId);
              if (!nr || nr.area === 0) continue;
              const nc = palette[nr.colorIndex];
              const nLum = 0.299 * nc.r + 0.587 * nc.g + 0.114 * nc.b;
              if (nLum - sLum <= 20) { allLighter = false; break; }
            }
          }
          if (allLighter) continue;
        }
      }
...
function faceAwareMergeSmallRegions(regionMap, regions, colorAssign, palette, width, height, minArea, faceMask, eyeMask, skinMask, edgeMap = null) {
  ...
  // ── Dark-feature protection ───────────────────────────────────────────
  function isDarkFeature(region) {
    if (region.area >= 400) return false;
    const col = palette[region.colorIndex];
    const lum = 0.299 * col.r + 0.587 * col.g + 0.114 * col.b;
    return lum < 80;
  }
...
function buildPbnSvg(regions, palette, width, height, colorAssign, showNumbers = true, showOutlines = true) {
...
const MainScreenPBY = () => {
  ...
  const [previewMode, setPreviewMode] = useState('rendering'); // 'rendering' | 'pbn' | 'paint'
  const [paintSvg, setPaintSvg] = useState(null);
    ...
    try {
      ...
      onApprove: async (data) => {
        ...
        try {
          ...
          if (result.success) { setOrderComplete(result.orderId || data.orderID); }
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      const paintString = buildPbnSvg(finalRegions, quantPalette, w, h, null, false, false);
      const outlineString = buildOutlineSvg(finalRegions, quantPalette, w, h);
      setPbnSvg(svgString);
      setPaintSvg(paintString);
      setOutlineSvg(outlineString);
  ...
  useEffect(() => {
    if (regions.length > 0 && palette.length > 0 && analysisWidth > 0) {
      ...
      const paintStr = buildPbnSvg(regions, quantPaletteRef.current, analysisWidth, analysisHeight, null, false, false);
      const outlineStr = buildOutlineSvg(regions, quantPaletteRef.current, analysisWidth, analysisHeight);
      setPbnSvg(svgStr);
      setPaintSvg(paintStr);
      setOutlineSvg(outlineStr);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="flex flex-col items-center text-center gap-1.5">
              ...
              <span className="text-sm font-bold text-gray-900 dark:text-white">Printed in Britain</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Made &amp; packed in our UK studio</span>
            </div>
            <div className="flex flex-col items-center text-center gap-1.5">
              <Truck className="h-7 w-7 text-orange-500" />
              <span className="text-sm font-bold text-gray-900 dark:text-white">Fast UK Delivery</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Dispatched within 3–5 working days</span>
            </div>
            <div className="flex flex-col items-center text-center gap-1.5">
              <Paintbrush className="h-7 w-7 text-orange-500" />
              <span className="text-sm font-bold text-gray-900 dark:text-white">Premium Cotton Canvas</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Gallery-grade stretched canvas</span>
            </div>
            <div className="flex flex-col items-center text-center gap-1.5">
              <Palette className="h-7 w-7 text-orange-500" />
              <span className="text-sm font-bold text-gray-900 dark:text-white">Up to 50 Acrylic Colours</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Full paint set included with every kit</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-1 space-y-6">
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-400 mt-2 flex items-center gap-1">
                    <AlertCircle className="h-3 w-3" />
                    More detail = more regions and more palette colours used. The higher the detail, the more difficult the painting will be, but the closer it will look to the original photo. Adjust this slider to find the right balance for your image and skill level. 
                  </p>
                </div>
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="border-t border-gray-200 dark:border-slate-700 pt-4" ref={checkoutSectionRef}>
                    ...
                    <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                      ...
                      <span className="text-xs text-blue-700 dark:text-blue-300 font-medium">Printed and shipped from Britain — fast UK delivery.</span>
                    </div>
            ...
            {orderComplete && (
              <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn">
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border-2 border-green-400 dark:border-green-600 overflow-hidden w-full max-w-md mx-4 animate-scaleIn">
                  <div className="p-8 bg-gradient-to-br from-green-500 via-emerald-500 to-teal-500 text-white text-center">
                    <div className="w-20 h-20 mx-auto mb-4 bg-white/20 rounded-full flex items-center justify-center ring-4 ring-white/30">
                      <Check className="h-10 w-10 text-white" strokeWidth={3} />
                    </div>
                    <h2 className="text-2xl font-bold">Payment Successful!</h2>
                    <p className="text-green-100 mt-1 text-sm">Your order has been placed</p>
                  </div>
                  <div className="p-8 text-center space-y-5">
                    ...
                    <div className="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-4 text-left">
                      ...
                      <p className="text-sm font-mono font-semibold text-gray-800 dark:text-gray-200 break-all">{orderComplete}</p>
                    </div>
                    <div className="flex flex-col gap-3">
                      <button
                        onClick={() => { setOrderComplete(null); setPbnSvg(null); setImageUrl(null); }}
                        className="w-full px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all font-semibold shadow-lg shadow-amber-500/25"
                      >
                        Create Another Design
                      </button>
                      <button
                        onClick={() => setOrderComplete(null)}
                        className="w-full px-6 py-2.5 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-sm font-medium transition-colors"
                      >
                        Close
                      </button>
                    </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  >
                    ...
                    >
                      {previewMode === 'rendering' && pbnSvg ? (
                        <div
                          className="max-w-full max-h-full"
                          style={{ userSelect: 'none', pointerEvents: 'none' }}
                          dangerouslySetInnerHTML={{ __html: pbnSvg }}
                        />
                      ) : previewMode === 'pbn' && outlineSvg ? (
                        <div
                          className="max-w-full max-h-full"
                          style={{ userSelect: 'none', pointerEvents: 'none' }}
                          dangerouslySetInnerHTML={{ __html: outlineSvg }}
                        />
                      ) : previewMode === 'paint' && paintSvg ? (
                        <div
                          className="max-w-full max-h-full"
                          style={{ userSelect: 'none', pointerEvents: 'none' }}
                          dangerouslySetInnerHTML={{ __html: paintSvg }}
                        />
                      ) : imageUrl ? (
                        <img
                          src={imageUrl}
                    ...
                    {pbnSvg && (
                      <div className="absolute top-4 left-4 flex items-center gap-2 z-20" onPointerDown={e => e.stopPropagation()}>
                        <button
                          onClick={() => setPreviewMode('rendering')}
                          className={`px-3 py-2 backdrop-blur-sm rounded-lg shadow-lg flex items-center gap-2 text-sm font-medium ${
                            previewMode === 'rendering'
              ...
              <>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                  <div className="relative">
                    ...
                    <div className="absolute top-3 right-3 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-lg px-3 py-1.5 shadow-md">
                      <span className="text-xs font-semibold text-gray-700 dark:text-gray-200 flex items-center gap-1.5">🇬🇧 Made in Britain</span>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow border border-gray-200 dark:border-slate-700 p-6">
            ...
            <div className="mt-3 flex items-start gap-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-3 py-2">
              ...
              <p className="text-xs text-blue-700 dark:text-blue-300">
                Every kit is printed and shipped from our studio in Britain — no long overseas waits. UK orders typically arrive within 3–5 working days.
              </p>
            </div>
          </div>
