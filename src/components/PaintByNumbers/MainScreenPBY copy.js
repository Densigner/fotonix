...
// ─── Master Palette (36 portrait-optimised colours) ─────────────────────────
...
const SKIN_END   = 16;  // 'Deep Skin Shadow' (inclusive)
...
function buildPbnSvg(regions, palette, width, height, colorAssign, showNumbers = true) {
  ...
  // Region boundary lines (where adjacent pixels belong to different regions)
  let bd = '';
  // Horizontal edges
  for (let y = 1; y < height; y++) {
    let segStart = -1;
    for (let x = 0; x < width; x++) {
      const above = regionIdMap[(y - 1) * width + x];
      const below = regionIdMap[y * width + x];
      if (above !== below) {
        if (segStart === -1) segStart = x;
      } else {
        if (segStart !== -1) { bd += `M${segStart},${y}H${x}`; segStart = -1; }
      }
    }
    if (segStart !== -1) bd += `M${segStart},${y}H${width}`;
  }
  // Vertical edges
  for (let x = 1; x < width; x++) {
    let segStart = -1;
    for (let y = 0; y < height; y++) {
      const left = regionIdMap[y * width + (x - 1)];
      const right = regionIdMap[y * width + x];
      if (left !== right) {
        if (segStart === -1) segStart = y;
      } else {
        if (segStart !== -1) { bd += `M${x},${segStart}V${y}`; segStart = -1; }
      }
    }
    if (segStart !== -1) bd += `M${x},${segStart}V${height}`;
  }
  if (bd) {
    svg += `<path d="${bd}" fill="none" stroke="#444" stroke-width="0.4" stroke-linecap="square" />`;
  }
...
function buildOutlineSvg(regions, palette, width, height, { a4 = false } = {}) {
  ...
  // Boundary lines only (where adjacent pixels belong to different regions)
  let bd = '';
  // Horizontal edges
  for (let y = 1; y < height; y++) {
    let segStart = -1;
  ...
  if (bd) {
    svg += `<path d="${bd}" fill="none" stroke="#000" stroke-width="0.5" stroke-linecap="square" />`;
  }
...
const MainScreenPBY = () => {
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      const finalPalette = usedIndices.map(i => MASTER_PALETTE[i]);
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                ...
                <div className="p-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    {palette.map((col, idx) => {
                      // Find regions using this colour
                      const regionNums = regions.filter(r => r.colorIndex === idx && r.area > 0).map(r => r.displayNumber);
