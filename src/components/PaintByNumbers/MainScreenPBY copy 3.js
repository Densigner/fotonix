...
/**
 * Loads the TinyFaceDetector model from /models/ (public folder).
 ...
 * Returns an array of bounding boxes: { x, y, width, height } in the
 * coordinate system of the provided element.
 */
...
/**
 * Build a binary face mask (Uint8Array, 1 = inside a face region, 0 = outside).
 * Each detected face bounding box is expanded by 1.6× around its center so that
 * hair, ears, and jaw line are included in the high-detail zone.
 *
 * Supports multiple faces in a single image.
 */
function buildFaceMask(faces, width, height) {
  ...
  for (const face of faces) {
    ...
    const x1 = Math.min(width, Math.ceil(cx + ew / 2));
    const y1 = Math.min(height, Math.ceil(cy + eh / 2));
    for (let y = y0; y < y1; y++) {
...
/**
 * Run two separate k-means passes — one on face pixels and one on non-face
 * pixels — then merge into a single unified palette.  Face regions get more
 * colours (baseK + 8) to preserve subtle tonal gradients.
 *
 * Returns the merged palette array.
 */
function faceAwareKMeansQuantise(imageData, baseK, faceMask) {
  const { data, width, height } = imageData;
  const totalPixels = width * height;

  // Partition pixels into face / non-face samples
  const faceSamples = [];
  const bgSamples = [];
  const sampleRate = Math.max(1, Math.floor(totalPixels / 50000));
  for (let i = 0; i < totalPixels; i += sampleRate) {
    ...
    if (faceMask[i]) faceSamples.push(px);
    else bgSamples.push(px);
  }

  // If no face pixels were sampled, fall back to standard quantisation
  if (faceSamples.length < 10) {
    return kMeansQuantise(imageData, baseK);
  }

  // Build synthetic ImageData for each zone
  function samplesToImageData(samples) {
    ...
    // ImageData constructor needs valid width×height; use Nx1 strip
    return new ImageData(buf, samples.length, 1);
  }

  const faceK = Math.min(baseK + 8, 48);  // more colours inside faces
  const bgK   = baseK;                     // normal palette for background

  const facePalette = kMeansQuantise(samplesToImageData(faceSamples), faceK);
  const bgPalette   = kMeansQuantise(samplesToImageData(bgSamples), bgK);

  // Merge palettes — deduplicate entries closer than ΔE < 4
  const merged = [...facePalette];
  for (const c of bgPalette) {
    const tooClose = merged.some(m => deltaE(m.lab, c.lab) < 4);
 ...
 */
function faceAwareMergeSmallRegions(regionMap, regions, colorAssign, palette, width, height, minArea, faceMask) {
  const paletteLab = palette.map(c => c.lab);
  ...
  function isFaceRegion(region) {
    ...
    for (const px of region.pixels) {
      if (faceMask[px]) faceCount++;
    }
    return faceCount / region.area > 0.5;
  }

  // ── Pass 1: merge small NON-FACE regions ──────────────────────────────
  const bgSmall = regions
    .filter(r => r.area > 0 && r.area < minArea && !isFaceRegion(r))
    .sort((a, b) => a.area - b.area);

  for (const small of bgSmall) {
    const neighbourLabels = new Map();
    for (const px of small.pixels) {
  ...
  // ── Pass 2: merge small FACE regions with a much lower threshold ──────
  const faceMinArea = Math.max(5, Math.floor(minArea * 0.3));
  const faceSmall = regions
    .filter(r => r.area > 0 && r.area < faceMinArea && isFaceRegion(r))
    .sort((a, b) => a.area - b.area);

  for (const small of faceSmall) {
    const neighbourLabels = new Map();
    for (const px of small.pixels) {
      const x = px % width, y = Math.floor(px / width);
      const neighbours = [];
...
const MainScreenPBY = () => {
  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      //    When faces are detected, use face-aware quantisation which runs
      //    separate k-means on face / non-face pixels with more colours
      //    allocated to faces, then merges the palettes.
      setProgressMsg('Quantising colours…');
      await new Promise(r => setTimeout(r, 30));
      const t2 = performance.now();
      const segK = Math.round(12 + (detailLevel / 100) * 24); // 12–36 based on detail
      const quantPalette = faceMask
        ? faceAwareKMeansQuantise(imageData, segK, faceMask)
        : kMeansQuantise(imageData, segK);
      logPipelineStats('3. K-Means Quantise', {
        uniqueColors: quantPalette.length,
        elapsed: performance.now() - t2,
        note: faceMask ? `face-aware: segK=${segK}, faceK=${Math.min(segK + 8, 48)}` : `segK=${segK}`
      });
      ...
      const cleaned = faceMask
        ? faceAwareMergeSmallRegions(seg.regionMap, seg.regions, seg.colorAssign, quantPalette, w, h, minArea, faceMask)
        : mergeSmallRegions(seg.regionMap, seg.regions, seg.colorAssign, quantPalette, w, h, minArea);
      logPipelineStats('7. Merge Small Regions', {
        ...
        note: faceMask
          ? `face-aware: bgMinArea=${minArea}, faceMinArea=${Math.max(5, Math.floor(minArea * 0.3))}`
          : `minArea=${minArea} (detailLevel=${detailLevel})`
      });
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-12" aria-label="About PaintYourPhoto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow border border-gray-200 dark:border-slate-700 p-6">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Download your finished design as a print-ready PNG or order a professionally printed canvas or paper kit delivered straight to your door — complete with numbered regions and colour key.
            </p>
          </div>
