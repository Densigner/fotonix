...
// clipper-lib is used to union and offset polygons when generating acrylic rings
...
export default function ProductPage() {
  ...
  try { window.__FOTONIX_FABRIC_LOADED = true; } catch(e){}
        ...
        setReady(true);
        // wire automatic acrylic ring updates so the ring follows user edits
        try {
          // wire and store unbind so we can toggle auto-update later
          const unbind = wireAcrylicAutoUpdate(c);
          try { c.__acrylic_unbind = unbind; } catch (e) {}
        } catch (e) {
          console.warn('wireAcrylicAutoUpdate failed', e);
        }
  ...
  // --- Acrylic ring helpers (Clipper-based) ---
  const ACRYLIC_SCALE = 100; // clipper works with integers
  ...
  function toWorldClipper(poly, obj) {
    ...
    return poly.map(pt => {
      ...
      return { X: Math.round(w.x * ACRYLIC_SCALE), Y: Math.round(w.y * ACRYLIC_SCALE) };
    });
  }
  ...
  async function objectToPolylinesAsync(obj, tolerance = 1.25) {
    if (!obj || obj._mirrorLayer || obj._safeArea || obj._acrylicRing) return [];
  ...
  // Simple wrapper to generate the acrylic ring on demand
  function borderMe(c, opts = {}) {
    try {
      if (!c) return null;
      // sensible defaults: expand a bit and create a visible ring
      const defaults = { expandPx: 18, ringPx: 14, tolerance: 6 };
      const params = Object.assign({}, defaults, opts);
      return makeAcrylicRing(c, params);
    } catch (e) { console.warn('borderMe failed', e); return null; }
  }

  function svgPathFromRings(outers, inners) {
    const seg = (poly) => poly.map((p, i) => `${i ? 'L' : 'M'} ${p.X / ACRYLIC_SCALE} ${p.Y / ACRYLIC_SCALE}`).join(' ') + ' Z';
    const parts = [];
    (outers || []).forEach(p => parts.push(seg(p)));
    (inners || []).forEach(p => parts.push(seg(p.reverse())));
    return parts.join(' ');
  }

  async function makeAcrylicRing(c, { expandPx = 12, ringPx = 10, tolerance = 1.25 } = {}) {
    const Clipper = window.ClipperLib || ClipperLib;
    if (!Clipper || !Clipper.Clipper) { console.warn('ClipperLib not available'); return null; }

    const users = c.getObjects().filter(o => !o._mirrorLayer && !o._safeArea && !o._acrylicRing);
    if (!users.length) { console.warn('No drawable objects'); return null; }

    let polys = [];
    for (const o of users) {
      try {
        const paths = await objectToPolylinesAsync(o, tolerance);
        if (paths && paths.length) polys.push(...paths);
      } catch (e) {
        /* ignore per-object failures */
      }
    }
    polys = polys.map(dedupe).filter(p => p.length >= 3);
    if (!polys.length) { console.warn('No valid polygons from objects'); return null; }

    const clipper = new Clipper.Clipper();
    clipper.AddPaths(polys, Clipper.PolyType.ptSubject, true);
    const unioned = new Clipper.Paths();
    clipper.Execute(Clipper.ClipType.ctUnion, unioned, Clipper.PolyFillType.pftNonZero, Clipper.PolyFillType.pftNonZero);
    if (!unioned.length) { console.warn('Union produced no paths'); return null; }

    const co = new Clipper.ClipperOffset(2, 2);
    co.AddPaths(unioned, Clipper.JoinType.jtRound, Clipper.EndType.etClosedPolygon);
    const outer = new Clipper.Paths();
    co.Execute(outer, expandPx * ACRYLIC_SCALE);

    const innerInset = Math.max(0, expandPx - ringPx);
    const coInner = new Clipper.ClipperOffset(2, 2);
    coInner.AddPaths(unioned, Clipper.JoinType.jtRound, Clipper.EndType.etClosedPolygon);
    const inner = new Clipper.Paths();
    coInner.Execute(inner, innerInset * ACRYLIC_SCALE);

    const seg = (poly) => poly.map((p,i)=>`${i?'L':'M'} ${p.X / ACRYLIC_SCALE} ${p.Y / ACRYLIC_SCALE}`).join(' ') + ' Z';
    const d = [ ...outer.map(seg), ...inner.map(p => seg([...p].reverse())) ].join(' ');

    const prev = c.getObjects().find(o => o._acrylicRing);
    if (prev) c.remove(prev);

    const ring = new window.fabric.Path(d, {
      fillRule: 'evenodd',
      fill: 'rgba(255,255,255,0.18)',
      stroke: 'rgba(255,255,255,0.35)',
      strokeWidth: 1.2,
      strokeUniform: true,
      shadow: { color: 'rgba(0,0,0,0.22)', blur: 8, offsetX: 0, offsetY: 1 },
      selectable: false,
      evented: false,
    });
    ring._acrylicRing = true;
    c.add(ring);
    c.sendToBack && c.sendToBack(ring);
    c.requestRenderAll && c.requestRenderAll();
    return ring;
  }

  function wireAcrylicAutoUpdate(c) {
    if (!c) return () => {};
    // debounce helper
    let timer = null;
    const debounce = (fn, wait = 80) => {
      return () => { if (timer) clearTimeout(timer); timer = setTimeout(fn, wait); };
    };

  const regenNow = () => { try { makeAcrylicRing(c).catch(e=>{ /* eslint-disable-next-line no-console */ console.warn('acrylic regen error', e); }); } catch (e) { /* eslint-disable-next-line no-console */ console.warn('acrylic regen error', e); } };
    const regen = debounce(regenNow, 120);
    const attach = () => {
      try {
        // detach any existing handlers we installed previously to avoid duplicates
        if (c.__acrylic_handlers && Array.isArray(c.__acrylic_handlers)) {
          try {
            c.__acrylic_handlers.forEach(h => { try { c.off && c.off(h.event, h.fn); } catch(e){} });
          } catch (e) {}
        }
        const wrapper = (ev) => { try { regen(); } catch (e) { /* swallow */ } };
        // keep explicit references so we can remove them later
        const hAdd = { event: 'object:added', fn: wrapper };
        const hMod = { event: 'object:modified', fn: wrapper };
        const hRem = { event: 'object:removed', fn: wrapper };
        c.__acrylic_handlers = [hAdd, hMod, hRem];
        if (c.on) {
          c.on(hAdd.event, hAdd.fn);
          c.on(hMod.event, hMod.fn);
          c.on(hRem.event, hRem.fn);
        }
      } catch (e) { /* ignore */ }
    };
    const detach = () => {
      try {
        if (c.__acrylic_handlers && Array.isArray(c.__acrylic_handlers)) {
          for (const h of c.__acrylic_handlers) {
            try { c.off && c.off(h.event, h.fn); } catch (e) {}
          }
        }
        c.__acrylic_handlers = null;
    ...
    // attach handlers
    attach();
    // run once immediately so enabling auto produces a ring right away
    try { regenNow(); } catch (e) {}
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 shadow-2xl ring-1 ring-black/5 backdrop-blur" style={{ display: 'inline-block' }}>
              ...
              <div className="px-5 pb-5">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3">
                  ...
                  <button className="px-4 py-2 rounded-lg bg-sky-600 hover:bg-sky-500 text-white font-medium shadow" onClick={() => { const c = fabricCanvasRef.current; if (!c) return; makeAcrylicRing(c).then(r=>{ if (!r) alert('No ring was generated — make sure you have drawable objects on the canvas'); }).catch(e=>{ console.warn(e); alert('Ring generation failed'); }); }}>Regenerate Ring</button>
                  <button className="px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white font-medium shadow" onClick={() => { const c = fabricCanvasRef.current; if (!c) return; borderMe(c) && Promise.resolve().then(()=>{}); }}>Border Me</button>
                  <button className={`px-3 py-2 rounded-lg ${autoRing ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-slate-700 hover:bg-slate-600'} text-white font-medium shadow`} onClick={() => {
                    const c = fabricCanvasRef.current; if (!c) return; try {
                      if (!autoRing) {
                        // enable auto wiring and store unbind
                        const unbind = wireAcrylicAutoUpdate(c);
                        try { c.__acrylic_unbind = unbind; } catch(e){}
                        setAutoRing(true);
                      } else {
                        // disable: call stored unbind if present
                        if (c.__acrylic_unbind) {
                          try { c.__acrylic_unbind(); } catch(e){}
                          c.__acrylic_unbind = null;
                        }
                        setAutoRing(false);
                      }
                    } catch (e) { console.warn('toggle auto ring failed', e); }
                  }}>{autoRing ? 'Auto Ring: On' : 'Auto Ring: Off'}</button>
