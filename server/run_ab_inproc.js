...
// Require the express app and start it in-process
...
async function run() {
  // Start server in-process on port 5002
  const port = 5002;
  const server = app.listen(port, '127.0.0.1');
  await new Promise((resolve) => server.once('listening', resolve));
  console.log('In-process openaiImageProxy listening on 127.0.0.1:' + port);

  try {
    const input = path.resolve(__dirname, '..', 'public', 'generated', 'generated-1757407267463-ilga33f.png');
    if (!fs.existsSync(input)) {
      console.error('Input file for AB runner not found:', input);
      server.close();
      process.exit(2);
    }
    const prompts = [
    ...
    for (const p of prompts) {
      for (const c of combos) {
        ...
        try {
          ...
          results.push({ prompt: p, opts: c, error: String(e) });
          continue;
        }
        let outPath = null;
        if (res && res.url) {
...
if (require.main === module) run().catch((e)=>{ console.error('AB inproc runner error', e); process.exit(1); });
